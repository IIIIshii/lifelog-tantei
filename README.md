...
ユーザーは**選択肢をタップ**するか、**短い文章（音声入力も可）**で答えるだけ。
...
| 探偵キャラクター選択 | 口調・質問文・日記の文体が異なる4種の探偵から選べる（後述） |
| AIとの対話 | 出来事を 5W1H（いつ/どこで/誰が/何が/どう感じた）で順に質問。選択肢は2択ならボタン、3択以上はドロップダウンで提示 |
| AI深堀り質問 | 構造化質問の後、Geminiが会話を読んで追加質問を最大3回生成。「証言十分」と判断すれば自動で打ち切る |
...
| 追記・作り直し | その日の日記が既にある場合、「追記する / いちから作り直す / 確認する」を選べる |
| 証拠保存 | 生成した日記・回答・数値データをFirestoreに保存 |
| 事件簿アーカイブ | 過去の日記をリスト表示（リアルタイム更新）し、タップで全文・編集 |
| 証拠分析室 | 直近14日のサマリー（平均睡眠・運動/勉強実施率・連続記録）、睡眠時間の棒グラフ、感情/場所の円グラフ、活動記録テーブル |
| AI所見 | 証拠分析室で「探偵に推理させる」と、Geminiが直近14日を読み返して所見（傾向・兆候・励まし）を生成・キャッシュ |
| 通知 | 設定した時刻に毎日ローカル通知（リマインダー）を送る |
| 設定（探偵事務所） | 探偵キャラ・記録項目・カスタム質問・テーマ・通知時刻を設定 |
| カスタム質問 | ユーザーが自由に質問を追加でき、対話と分析表に反映される |
| テーマ切替 | 3種のテーマを切替（端末ローカルに永続化） |
| CSVエクスポート | 全エントリ（日付・日記・回答）をBOM付きCSVに書き出して共有 |
| デモ | ゲストログイン＋サンプル事件簿7日分の一括投入（ログイン画面 / デバッグ設定） |

---

## 探偵キャラクター（ロール）

設定画面で選んだロールにより、質問文・相槌・生成される日記の文体が変わります。
定義は `lib/roles/` に1ファイル1キャラで分離され、`lib/roles/roles.dart` のレジストリに登録されています。

| キー | 名称 | 特徴 |
|------|------|------|
| `hardboiled` | ハードボイルド探偵 | 寡黙でクール。硬質・簡潔な三人称の捜査ログ調（デフォルト） |
| `novice` | 新卒探偵 | 入社1日目。ぎこちない丁寧語で一生懸命 |
| `alien` | エイリアン | 地球を観察中の宇宙人。カタカナ混じりの観測記録調 |
| `psychologist` | 心理学者 | 穏やかで内省的。事実より「何を感じたか」を掘り下げる |

新しいロールは `lib/roles/<name>.dart` を追加し、`roles.dart` の `kRoles` に登録するだけで増やせます（設定画面の一覧もレジストリを参照するため自動反映）。
...
5. AIによる深堀り質問（Geminiが会話を読み、最大3回 / 十分なら打ち切り）
...
- **テーマ** — 3種から選択（ライブプレビュー付き）
- **通知設定** — 毎日リマインダーのON/OFFと時刻
- **データ管理** — CSVエクスポート
...
| 認証 | Firebase Auth（Google / 匿名）+ google_sign_in |
| DB | Cloud Firestore |
...
プロジェクトルートに `.env` を作成します（`pubspec.yaml` の assets に登録済み）。

```
GEMINI_API_KEY=your_gemini_api_key_here
# Googleログインを使う場合のみ必須（google-services.json の client_type:3 のID）
GOOGLE_WEB_CLIENT_ID=your_firebase_web_client_id_here
```
...
用途ごとに `GenerativeModel` を分け、それぞれに専用のシステム指示をセットします。
...
- **所見生成** — `AiInstructions.analyst()` ＋ ロールの `analystStyle`

### テーマ／ロールの拡張ポイント

- **テーマ追加** — `app_theme.dart` の `AppThemeName` と `colorsOf()` に追加
- **ロール追加** — `lib/roles/<name>.dart` を作り `roles.dart` の `kRoles` に登録（設定一覧は自動反映）
